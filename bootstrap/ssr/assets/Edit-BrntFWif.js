...
var _sfc_main = {
	...
	setup(__props) {
		...
		const thumbnailUrl = ref(props.post.thumbnail_url ?? null);
		function submit() {
			form.put(`/internal/posts/${props.post.id}`);
		}
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<!--[-->`);
			_push(ssrRenderComponent(unref(Head), { title: `Edit — ${__props.post.title}` }, null, _parent));
			_push(ssrRenderComponent(_sfc_main$1, null, {
				header: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<div${_scopeId}><h1 class="text-xl font-semibold text-warm-900 font-display"${_scopeId}>Edit Post</h1><p class="text-sm text-warm-500 mt-0.5 truncate max-w-xs"${_scopeId}>${ssrInterpolate(__props.post.title)}</p></div></div>`);
						...
						_: 1
					}), createVNode("div", null, [createVNode("h1", { class: "text-xl font-semibold text-warm-900 font-display" }, "Edit Post"), createVNode("p", { class: "text-sm text-warm-500 mt-0.5 truncate max-w-xs" }, toDisplayString(__props.post.title), 1)])])];
				}),
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
